-- =====================================================================
-- BERSYON PARA SA PRODUCTION (Task 7 A1) -- binuo ng migrations/scratch/make_prod_suite.mjs mula sa giving_rls_tests.sql.
-- Huwag i-edit nang mano-mano; patakbuhin muli ang generator.
-- Isang transaction lang (walang BEGIN/COMMIT sa file); laging nagtatapos sa sadyang exception kaya NABABAWI ang lahat.
-- Walang dblink/pg_net/http/email. Walang DELETE/UPDATE sa financial_records, at ang mga UPDATE/DELETE sa profiles at
-- ministry_members ay sa mga sintetikong id lang (ang INV na pagsusuri sa dulo ang nagpapatunay).
-- =====================================================================
-- =====================================================================
...
-- (production) mga kopya ng TOTOONG row bago ang anumang fixture
create temp table snap_profiles as select * from public.profiles;
create temp table snap_ministries as select * from public.ministries;
create temp table snap_ministry_members as select * from public.ministry_members;
create temp table snap_financial_records as select * from public.financial_records;
create temp table snap_abuluyan_totals as select * from public.abuluyan_totals;
create temp table snap_audit_log as select * from public.audit_log;
create temp table snap_finance_audit_log as select * from public.finance_audit_log;
create temp table snap_members as select * from public.members;
create temp table snap_giving_permissions as select * from public.giving_permissions;
create temp table snap_app_config as select * from public.app_config;
...
begin
  ...
  -- (production) inalis ang m1-m4: tumatarget ang mga ito sa totoong Finance/Local Finance/Pastoral Ministry
  ...
  -- (production) inalis ang m12: nag-uupdate ng totoong row ng Finance Ministry
  ...
  -- ---------------- (production) INV: walang totoong row na nabago o nabura ----------------
  for r in select x from unnest(array['profiles', 'ministries', 'ministry_members', 'financial_records', 'abuluyan_totals', 'audit_log', 'finance_audit_log', 'members', 'giving_permissions', 'app_config']) as x loop
    execute format('select count(*) from (select * from pg_temp.snap_%s except select * from public.%s) q', r.x, r.x) into v;
    perform pg_temp.rec('INV: ang totoong row ng ' || r.x || ' ay hindi nabago o nabura', v = 0, 'nabago/nabura=' || v);
  end loop;
