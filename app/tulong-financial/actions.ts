...
export async function forwardLoanRequestToAdmin(fd: FormData): Promise<never> {
  ...
  // Kunin ang detalye para sa Telegram notification.
...
export type UsernameRequestResult = {
  ...
  tempPassword?: string;
  memberName?: string;
  // True kapag ang kaanib ay may portal account na (walang bagong credentials).
  portalUser?: boolean;
  error?: string;
};
...
// Hanapin ang portal account ng isang kaanib (gamit ng form bago isumite,
// para makumpirma ng Finance Ministry). Bumabalik ng null kung wala.
export async function lookupPortalAccount(
  memberId: string,
): Promise<{ profile_id: string; full_name: string; email: string | null; via: string } | null> {
  const { supabase } = await requireTulongFinancialAccess();
  if (!memberId) return null;
  const { data } = await supabase.rpc("tulong_find_portal_profile", { p_member_id: memberId });
...
export async function requestTulongUsername(fd: FormData): Promise<UsernameRequestResult> {
  ...
  const memberName = (member as any).full_name as string;

  // Tiyakin muna: kung may portal account na ang kaanib (at pinili ng Finance
  // Ministry na gamitin ito), approval letter lang ang kailangan — walang
  // bagong username/password na lilikhain. Ang paghahanap ay inuulit dito
  // sa server (hindi pinagkakatiwalaan ang ipinasa ng client).
  const usePortal = String(fd.get("use_portal") ?? "") === "1";
  let portalProfile: { profile_id: string; full_name: string; email: string | null } | null = null;
  if (usePortal) {
    const { data } = await supabase.rpc("tulong_find_portal_profile", { p_member_id: memberId });
    portalProfile = (data as any) ?? null;
  }

  if (portalProfile?.profile_id) {
    const portalProfileId = portalProfile.profile_id;
    ...
    const { error: e2 } = await supabase.rpc("tulong_send_request_letter", {
      ...
      p_subject: `Kahilingan ng pag-apruba — ${memberName}`,
      p_body:
        `Humihiling ang Finance Ministry ng pag-apruba para kay ${memberName}. ` +
        `May portal account na ang kaanib — gagamitin niya ang kanyang portal login, kaya walang bagong ` +
        `username/password na lilikhain. Pakitingnan sa ibaba: Aprubahan o Tanggihan. ` +
        `Kapag na-aprubahan, maaari na siyang mag-login sa idbcj.org/tulong-financial gamit ang portal account.`,
    });
    ...
    return { ok: true, memberName, portalUser: true };
  }

  const username = await uniqueUsername(supabase, usernameBaseFromName(memberName ?? "", memberId));
  ...
  await notifyTelegram({
    kind: "username",
    member_name: (member as any).full_name,
    username,
  });
