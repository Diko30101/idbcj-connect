...
export async function submitLoanRequestAsPortalUser(fd: FormData): Promise<never> {
  ...
  const supabase = await supa();
  const { data, error } = await supabase.rpc("tulong_create_loan_request_by_profile", {
    p_amount: Math.round(amount * 100) / 100,
    p_target_return_date: /^\d{4}-\d{2}-\d{2}$/.test(target) ? target : null,
    p_notes: notes || null,
  });
...
export async function changeBorrowerPassword(fd: FormData): Promise<never> {  const token = await getBorrowerSessionToken();
  if (!token) redirect(`${BASE}/login`);
