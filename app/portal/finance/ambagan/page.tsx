...
import { createAmbagan, searchMembers, submitAmbagan, submitManyAmbagan, updateAmbagan } from "./actions";
...
export default async function AmbaganPage({
  ...
  const path = `${GIVING_BASE}/ambagan${ctx.isChurch && selected ? `?local=${selected.id}` : ""}`;
  ...
  return (
    <>
      <PageHeader
        title="Ambagan"
        subtitle={selected ? `Pag-e-encode ng ambag ng bawat kaanib · ${selected.name}` : "Pag-e-encode ng ambag ng bawat kaanib"}
      />
      ...
      {ctx.isChurch && locals.length > 1 && (
        <div className="mt-6 flex flex-wrap gap-2">
          {locals.map((l) => (
            <Link
              key={l.id}
              href={`${GIVING_BASE}/ambagan?local=${l.id}`}
      ...
      {selected && (
        <>
          <div className="mt-6">
            <Panel title="Bagong Ambagan">
              <AmbaganForm
                action={createAmbagan}
          ...
          <div className="mt-6">
            <Panel title="Mga Naitalang Ambagan">
              <AmbaganList
                ...
                actions={{ update: updateAmbagan, submit: submitAmbagan }}
